...
def searchPost(search_tags):
    ...
    # Initialize empty lists to store post and user data
    posts = []
    users_list = []
    user_likes = []
    
    for i in results:
        # Fetch all the matching posts for a given tag
        ...
        users_list.append([username, display, pfp])
        ...
        if g.user is None:
            ...
            cursor.execute(''' SELECT postID FROM likes WHERE userID = %s;''', (getUserID(cursor, g.user)))
            likes = cursor.fetchall()
            likes = [i[0] for i in likes]
            user_likes.append(likes)

    results = len(results)
    return render_template("search.html", page = "Search", results = results, posts = posts, user_list = users_list, user_likes=user_likes)
